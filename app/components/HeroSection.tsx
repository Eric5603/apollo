...
export default function ModernConstructionHero() {
  ...
  return (
    <section ref={containerRef} className="min-h-screen bg-white relative overflow-hidden isolate">
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
              {[{ value: "98%", label: "Safety Record" },
                ...
                { value: "24/7", label: "Site Monitoring" }].map((stat, index) => (
                <motion.div
                  key={index}
                  whileHover={{ y: -5 }}
            ...
            <div className="flex flex-wrap gap-4 mt-8">
              <Link 
                href="/start-project" 
                className="px-6 py-4 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 transition-colors text-base shadow-lg"
              >
                Start New Project
              </Link>
              <Link 
                href="/innovations" 
                className="px-6 py-4 border-2 border-gray-900 text-gray-900 rounded-lg font-bold hover:border-blue-600 hover:text-blue-600 transition-colors text-base shadow-sm"
              >
                View Innovations
              </Link>
            </div>
